...
//			this.rollback(conn);
	 ...
	 */
	public <T> T queryForOneClass(Class<T> clazz, String sql, Object... args) {
		Connection conn = null;
		...
		try {
			...
			if (rs.next()) {
				T t = clazz.newInstance();// 通过反射获取实例对象
				// 处理结果集一行数据中的每一列
	 ...
	 */
	public <T> List<T> queryForListClass(Class<T> c, String sql, Object... args) {
		Connection conn = null;
		PreparedStatement ps = null;
		ResultSet rs = null;
